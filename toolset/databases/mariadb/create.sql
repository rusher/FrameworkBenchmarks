...
CREATE USER 'benchmarkdbuser'@'%' IDENTIFIED WITH mysql_native_password USING PASSWORD('benchmarkdbpass');
CREATE USER 'benchmarkdbuser'@'localhost' IDENTIFIED WITH mysql_native_password USING PASSWORD('benchmarkdbpass');
GRANT SELECT, UPDATE ON hello_world.* TO 'benchmarkdbuser'@'%';
GRANT SELECT, UPDATE ON hello_world.* TO 'benchmarkdbuser'@'localhost';
FLUSH PRIVILEGES;
